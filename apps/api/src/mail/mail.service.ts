...
export class MailService {
  ...
  async sendNewHireAnnouncement(
    ...
    await this.send({
      to: to[0],
      subject: `Welcome ${fullName} to the team! 🎉`,
      text: `Please join us in welcoming ${displayName} to 1Solutions. ${[input.designation, input.department].filter(Boolean).join(', ')} Email: ${input.workEmail}${input.phone ? ` Phone: ${input.phone}` : ''}`,
      html: wrapCompanyEmail(bodyHtml),
      bcc: to.slice(1),
    });
  }
