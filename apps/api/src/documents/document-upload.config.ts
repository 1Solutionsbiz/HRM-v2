 ...
 */
export function employeeDocumentsDir(): string {
  return resolve(process.env.UPLOADS_DIR ?? './uploads', 'employee-documents');
}
...
export const employeeDocumentMulterOptions: MulterOptions = {
  storage: diskStorage({
    destination: (_req, _file, cb) => {
      const dir = employeeDocumentsDir();
  ...
  fileFilter: (_req, file, cb) => {
    if (!ALLOWED_MIME_TYPES.has(file.mimetype)) {
      cb(new BadRequestException('Documents must be a JPEG, PNG, WEBP, or PDF file.'), false);
      return;
    }
    cb(null, true);
  },
...
export function employeeDocumentFilePath(filename: string): string {
  return join(employeeDocumentsDir(), filename);
}
